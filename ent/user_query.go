...
func (uq *UserQuery) QueryStudies() *ListQuery {
	query := &ListQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, user.StudiesTable, user.StudiesPrimaryKey...),
		)
...
func (uq *UserQuery) QuerySettings() *SettingsQuery {
	query := &SettingsQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, true, user.SettingsTable, user.SettingsColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) WithStudies(opts ...func(*ListQuery)) *UserQuery {
	query := &ListQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withStudies; query != nil {
		if err := uq.loadStudies(ctx, query, nodes,
			func(n *User) { n.Edges.Studies = []*List{} },
			func(n *User, e *List) { n.Edges.Studies = append(n.Edges.Studies, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadStudies(ctx context.Context, query *ListQuery, nodes []*User, init func(*User), assign func(*User, *List)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.StudiesTable)
		s.Join(joinT).On(s.C(list.FieldID), joinT.C(user.StudiesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.StudiesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.StudiesPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "studies" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
